...
import cn.jeremy.wechat.stock.bean.CountStockTradeData;
...
public class ContinuousStockFundMrTextToDB extends BaseMrTextToDB<CountStockTradeData> {
    ...
    @Override
    public void insertDB(List<CountStockTradeData> result) {
    ...
    @Override
    public CountStockTradeData lineToObject(String line) {
        if (StringUtils.isEmpty(line)) {
            return null;
        }
        String[] split = line.split("\t");
        if (null != split && split.length > 2) {
            CountStockTradeData data = new CountStockTradeData(split[1], split[0]);
